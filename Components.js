...
spotter.components = (function(){
	...
	// *** OPERATIONAL/HELPERS ***
		...
		function isPartOfTemplate(elem){
			...
			var p=elem.parentNode, d=document, b=document.body;
			while(p && p!==b && p!==d){
				if(p.tagName === 'SPOTTER-TEMPLATE'){
					console.warn('custom element is part of a template',elem);
					return true;
				}
				p = p.parentNode;
			}
		...
		function registerElem(elem){
			...
			if(~registeredNodes.indexOf(elem)){
				console.warn('custom element already registered:','<'+elem.tagName+' '+'id="'+elem.id+' class="'+elem.className+'"');
				return false;
			}
			else{
		...
		//fires when components polyfill downloaded trigger event...
		...
		//check if component (not specific node) has been registered as a web component
		__components.isComponentRegistered = function(tagName){
			return __private.readyComponents[tagName] === true;
		};
		
		//register component type (not specific nodes)
		...
		//trigger function once node is registered with 'isReady' - 'this' will refer to the custom node		
		__components.onReady = function(node, func){
			...
			if(~i){//node previously readied
				...
				if(~(i = registeredNodes.indexOf(node))){//node was registered but not readied
					...
					console.debug('function added to trigger for previously registered node');
				}
				else{//node is unregistered
					i = onReadyNodesTemp.push(node) - 1;
					onReadyFuncs[i] = [func];
					console.debug('node is now registered and function added to trigger when node is ready');
				}
			}
		};
	
		//register a specific custom node as ready
		__components.isReady = function(node){
			if(!node && this.tagName) node = this;
	...
	// *** COMPONENTS ***
			...
			//(attr) data-option-template="options template name" - defaults to "modal-select" - the template to use for the individual options
				...
				__factory.setupMenu = function(cont, input){
					...
					var selected=[]//currently selected option(s)
						...
						,deselect = (cont.getAttribute("deselect") !== "disabled")
						,placeholder = cont.getAttribute("placeholder")
						,textTarget;
					
					//where selection choice will be shown
					if(textTarget = cont.getAttribute('text-target')){
					...
					if(textTarget !== null && placeholder !== null){
						spotter.placeholder.add(spotter.getFirstTextNode(textTarget), placeholder);
					}
					...
					return function(modalMenuVars){
						...
						var setSelected = function(){
							var c;
		//console.log('setSelected: \n', 'selected: ', selected, ' \n', 'multiSelect: ', multiSelect, ' \n', 'in selected?:', ~selected.indexOf(this), ' \n', 'deselect: ', deselect, ' \n', 'textTarget:', textTarget);
							if(~(c = selected.indexOf(this))){//previously selected
								...
								if(textTarget){ 
									var node = spotter.firstTextNodeRemoveValue(textTarget, this.parseData.text, placeholder);
									if(placeholder !== null) spotter.placeholder.add(node, placeholder);
								}
							}
							else{
								if(
									...
									if(textTarget){
										var node = spotter.setFirstTextNode(textTarget, this.parseData.text);
										if(placeholder !== null) spotter.placeholder.add(node, placeholder);
									}
								}
								else{
									...
									spotter.toggle.class.add(this,'selected');
									if(textTarget){
										var node = spotter.firstTextNodeAddValue(textTarget, this.parseData.text);
										if(placeholder !== null) spotter.placeholder.add(node, placeholder);
									}
								}
							}
				...
				var __constructor = function(cont){
					...
					console.log('modal select: ',cont.id);
					...
					var exclusive = null
						...
						,inputName = cont.getAttribute('name')
						,targInput = getElemByIdOrBuildIfFail(inputId, '<input type="hidden" '+(inputName!==null?'name="'+inputName+'"':'')+'/>')
					...
					if((typeof targInput !== "object") || (targInput.tagName !== 'INPUT' && !(targInput = targInput.querySelector('input')))){
						console.warn('target for modal select must either be an input or contain an input');
						return false;
					}
					...
					// * setup options *
					if(opts === null || opts.length === 0) opts = cont.getAttribute('spotter-options');console.log('spotter module opts',opts);
					
					if(typeof opts === "string"){
						var pos, rows = [];
						if(opts === "months"){
							rows[0] = {value:1, text:'january'};
							rows[1] = {value:2, text:'february'};
							rows[2] = {value:3, text:'march'};
							rows[3] = {value:4, text:'april'};
							rows[4] = {value:5, text:'may'};
							rows[5] = {value:6, text:'june'};
							rows[6] = {value:7, text:'july'};
							rows[7] = {value:8, text:'august'};
							rows[8] = {value:9, text:'september'};
							rows[9] = {value:10, text:'october'};
							rows[10] = {value:11, text:'november'};
							rows[11] = {value:12, text:'december'};
							
							//static modal menu
							popMenu = new spotter.popMenu('modal-select-'+inputId)
								.useTemplate(optTemplate)
								.bindDataToParseTemplate(rows);
						}
						else if(~(pos = opts.indexOf(' to '))){
							var min = opts.slice(0, pos)-1
								,max = opts.slice(pos+4)-0
								,dir = 1
								,x = 0
								,rows = [];
							
							while(++min <= max){
								rows[x++] = {value:min, text:min};
							}
						...
						/*
						else if(opts.indexOf('function') === 0){
							pos = opts.lastIndexOf(':');
						}
						*/
						else{
							rows = opts.split(',');
							rows.forEach(function(v,i){rows[i] = {value:v,text:v};});
					...
					else if(opts !== null && (o = opts.length) > 0){//if spotter-options are given, use them
					...
					var _pm = popMenu.onMenu(__factory.setupMenu(cont,targInput))
						.attachTo(cont)
						.loadTemplate();
						
					_pm.addEventListener('afterOpen', (function(targInput, popMenu){
						return function(arg){
							targInput.formerValue = targInput.value;
						};
					}(targInput, popMenu)));
					_pm.addEventListener('afterClose', (function(targInput){
						return function(){
				...
				__constructor.afterClose = function(el,func){
					var i;
					if((i = __factory.instances.indexOf(el)) > -1){
						var newFunc = function(){ func.call(__factory.instances[i],__factory.inputs[i]) };
						__factory.menus[i].afterClose(newFunc);
					}
				};
				
				__constructor.afterOpen = function(el,func){
					var i;
					if((i = __factory.instances.indexOf(el)) > -1){
						var newFunc = function(){ func.call(__factory.instances[i],__factory.inputs[i]) };
						__factory.menus[i].afterOpen(newFunc);
					}
				};	
				...
				*/
				registerSpotterComponent('spotter-modal-select', {
					prototype: Object.create(
						HTMLDivElement.prototype,
						{attachedCallback: {value: __constructor}}
					)
				});
		...
		if(deleted.length === 0) console.log('deleteListingsByFieldValue: \n', 'no listings found', ' \n', 'not found: ', this.dataManager.search.notFound, ' \n', 'arguments: ', arguments);
							...
							// ** SETUP **
								...
								if(templateSource = el.getAttribute('template')){
									setupOpts.template = templateSource;
								}
								else if(templateSource = el.querySelector('spotter-template')){
									setupOpts.template = templateSource;
								}
									...
									console.warn('ajax list must contain a template or else have the id of a template or import specified',el);
									return false;
								}
			...
			//if only the button is sent, then a textarea will be created within the button and the text will show through the first textnode of the button element.		
				...
				__constructor = function(cont){
					...
					var textBoxModal = new spotter.popMenu(targInput.name)
						...
						/*
						.afterClose( __factory.functions.getOnClose(button, targInput) )
						.afterOpen(function(){ 
					...
					textBoxModal.addEventListener('afterClose', __factory.functions.getOnClose(button, targInput));
					textBoxModal.addEventListener('afterOpen', function(){ 
						if(targInput.value !== null && targInput.value.length >= 0) { 
							targInput.modalRepresentative.value = targInput.value || null;
							targInput.modalRepresentative.hasBeenActivated = true;
						} 
			...
			__components.keypad = (function(){
				...
				var __constructor = function(keypadParent){
					...
					keys.forEach(function(key){
						...
						//animation events
						key.addEventListener(spotter.events.eventTypes.mouse.mouseup, function(){
							this.className=this.className+' active';
						}, false);
						key.addEventListener(spotter.events.eventTypes.mouse.mousedown, function(){
		...
		// -- CALENDAR --
			__components.calendar = (function(){	
				...
				var __constructor = function(cont){
					...
					/* -------------------MANUAL YEAR-------------------- */
					/*
					if(cont.hasAttribute('manual-date')){
						var yearInput=document.createElement('INPUT');
						yearInput.type="text";
						yearInput.className="user-text manual-year";
						yearInput.placeholder="enter date";
						selectedDate.addEventListener('click', function(){
							spotter.toggle.show(yearInput);
							yearInput.focus();
						});
						yearInput.addEventListener('blur', function(){
							spotter.toggle.hide(this);
						});
						yearInput.addEventListener('keydown', function(e){
							var key = spotter.keyboard.keyCodes[e.keyCode];
							if(!(spotter.isType.integer(key) || key === "/" || key === "-")){
								
							}
						});
						yearInput.addEventListener('keyup', function(e){
							var v = this.value.replace(/\D/g,''), l = v.length;
							if(l === 4){
					...
					*/
					...
					var cycleDays = function(){
						...
						if(daysFromSunday > 0){
							var daysInPreviousMonth=moment((month===0?year-1:year)+'-'+(month===0?12:month),'YYYY-M').daysInMonth(), targ;
							...
							for(x=0; x<daysFromSunday; x++){
								targ = inactiveDaysStart[7-previousOffsetDays-x];
								if(typeof targ === "undefined"){
									console.warn('inactivedaysend['+(7-previousOffsetDays-x)+'] was undefined');
								}
								else{
						...
						var diff=42 - (daysFromSunday + daysInCurrentMonth), targ;
						//if(diff < 0) diff = 42 - (daysFromSunday + daysInCurrentMonth);
						for(x=0; x<diff; x++){
							targ = inactiveDaysEnd[x];
							if(typeof targ === "undefined"){
								console.warn('inactivedaysend['+x+'] was undefined');
							}
							else{
								targ.style.display='inline-block';
							}
		...
		// -- SCHEDULER --
			__components.scheduler = (function(){
				//blocksInfo: [[blockStartTimeUTC,blockEndTimeUTC,*active], [blockStartTimeUTC2,blockEndTimeUTC2,*active]...]]
				//*active - 0|1 - 0 if block is disabled and 1 if block is active
				//daysPerSlide - (integer) - number of days to be shown per slide action - width of scheduler should be equal to daysPerSlide * widthColumn
				//set attribute 'blocks-info-url' to retrieve the schedule blocks remotely - set attribute 'days-per-slide' to set the number of days that will show per slide
				var ccc = 0;
				var __preConstructor = function(cont, blocksInfo, daysPerSlide){
					if(++ccc > 20){
						return false;
					};
					//console.log(' *************** __preConstructor **************** ');
					
					if(typeof cont === "undefined") cont = this;
					var dependenciesClear = true,
						requireMoment = 'N/A',
						requireVelocity = 'N/A';

					if(typeof moment !== "function"){
						if((requireMoment = spotter.require('moment', '/js/spotter/third_party/moment.js', [], function(){return moment;})) !== null){
							spotter.testLoaded(function(){
								console.log('moment activate');
								__preConstructor(cont, blocksInfo, daysPerSlide);
							}, 'moment');
						}
					...
					if(typeof Velocity !== "function"){
						if((requireVelocity = spotter.require('velocity', '/js/spotter/third_party/velocity.js', [], function(){return Velocity;})) !== null){
							spotter.testLoaded(function(){
								console.log('velocity activate');
								__preConstructor(cont, blocksInfo, daysPerSlide);
							}, 'velocity');
						}
						dependenciesClear = false;
					}
					//console.log('__components.scheduler -- \n', 'count:', ccc, 'typeof moment:', typeof moment, 'typeof Velocity', typeof Velocity, ' \n', 'require momentjs result:', requireMoment, 'require velocityjs result:', requireVelocity, '-- \n',new Error().stack);
					//console.log(' *************** END **************** ');
					if(dependenciesClear === true) __constructor(cont, blocksInfo, daysPerSlide);
				}
				
				var __constructor = function(cont, blocksInfo, daysPerSlide){					
					if(isPartOfTemplate(cont)) return;
					if(!registerElem(cont)) return;
					
					if(!blocksInfo) blocksInfo = cont.getAttribute('blocks-info-url');
					if(!daysPerSlide) daysPerSlide = cont.getAttribute('days-per-slide');
					
					if(typeof blocksInfo === 'string'){
						spotter.ajax({
							url: blocksInfo,
							success: function(response){
								blocksInfo = JSON.parse(response);
								if(blocksInfo.status === 'error'){
									console.warn('server returned an error for scheduler remote request');
									return false;
								}
								else{
									__components.scheduler.build(cont, blocksInfo.result, daysPerSlide);
								}
							},
							error: function(){
								console.warn('ajax request for scheduler info failed');
								return false;
							}
						});
					}
					else if(Array.isArray(blocksInfo)){
						__components.scheduler.build(cont, blocksInfo, daysPerSlide);
					}
					else{
						console.warn('argument blocksInfo sent to scheduler constructor must be a url string or an array');
						return false;
					}
				};
				
				registerSpotterComponent('spotter-scheduler', {
					prototype: Object.create(
						HTMLDivElement.prototype,
						{
							createdCallback: {value: __preConstructor}
			...
			//activated after getting block info
			__components.scheduler.build = function(cont, blocksInfo, daysPerSlide){
				if(!(blocksInfo.length > 0)){
					console.warn('empty set sent to scheduler builder');
					return false;
				}
				
				html = '<div class="slider-cont"><div class="slider"></div></div>'+
					'<div class="arrow-cont left content-centered"><div class="left-arrow centered-content disabled"></div></div>'+
					'<div class="arrow-cont right content-centered"><div class="right-arrow centered-content"></div></div>';
				
				cont.innerHTML = html;
				var slider = cont.slider = cont.querySelector('.slider'),
					leftArrow = cont.querySelector('.left-arrow'),
					rightArrow = cont.querySelector('.right-arrow');
				
				//build slider
				var _t = __components.scheduler.templates;
				var columnTemplate = _t.column;
				var labelTemplate  = _t.label;
				var blockTemplate  = _t.activeBlock;
				
				var previousDay = null;
				var schedulerHTML = "";
				var label;
				var blocks = '';
				blocksInfo.forEach(function(blockUTC){
					var dateTimeStart = moment.unix(blockUTC[0]),
						dateTimeEnd = moment.unix(blockUTC[1]),
						day = dateTimeStart.day();
					if(day !== previousDay){
						if(previousDay !== null) schedulerHTML += columnTemplate.replace('$', label+blocks);
						label = labelTemplate.replace('$', dateTimeStart.format('ddd MMM-DD'));
						blocks = '';
						previousDay = day;
					}
			
					blocks += blockTemplate.replace('$1', blockUTC[0]).replace('$2', dateTimeStart.format('hh:mmA')+' to '+dateTimeEnd.format('hh:mmA')).replace('$3', (blockUTC[2] == '0'?'disabled':'active'));
				});
				if(previousDay !== null) schedulerHTML += columnTemplate.replace('$', label+blocks);//add the last day parsed
				slider.innerHTML = schedulerHTML;

				cont.currentSlide = 0;
				cont.offsets = [];
				cont.setSliderOffsets = function(slider){
					var scheduler = this, c = 0;
					Array.prototype.slice.call(slider.children,0).forEach(function(child){
						scheduler.offsets[c++] = child.offsetLeft;
					});
					scheduler.numSlides = c;
					//console.log(scheduler.offsets);
				}.bind(cont, slider);
				cont.setSliderOffsets();
				
				leftArrowClickEvent = (function(cont, rightArrow, leftArrow){
					leftArrow.active = false;
					return function(){
						if(leftArrow.active === true){
							if(cont.shift(-3) === false){
								leftArrow.active = false;
								spotter.toggle.class.add(leftArrow, 'disabled');
							}
							else if(rightArrow.active === false){
								rightArrow.active = true;
								spotter.toggle.class.remove(rightArrow, 'disabled');
							}
						}
					};
				}(cont, rightArrow, leftArrow));
				leftArrow.addEventListener('click', leftArrowClickEvent);
				
				rightArrowClickEvent = (function(cont, leftArrow, rightArrow){
					rightArrow.active = true;
					return function(){
						if(rightArrow.active === true){
							if(cont.shift(3) === false){
								rightArrow.active = false;
								spotter.toggle.class.add(rightArrow, 'disabled');
							}
							else if(leftArrow.active === false){
								leftArrow.active = true;
								spotter.toggle.class.remove(leftArrow, 'disabled');
							}
						}
					};
				}(cont, leftArrow, rightArrow));
				rightArrow.addEventListener('click', rightArrowClickEvent);
				
				var c = 0;
				cont.maxSlide = (Math.ceil(cont.numSlides/daysPerSlide) * daysPerSlide) - daysPerSlide;//should not exceed this index because of the number of days per slide
				cont.shift = (function(scheduler, daysPerSlide){
					return function(slides){
						slides = slides || 0;
						var c = (scheduler.currentSlide+=slides), r = true;
			
						if(c >= cont.maxSlide){
							c = scheduler.currentSlide = cont.maxSlide;
							r = false;
						}
						else if(c <= 0){
							c = scheduler.currentSlide = 0;
							r = false;
						}
						Velocity(scheduler.slider, {left:'-'+scheduler.offsets[c]+'px'}, {duration:(200 * Math.abs(slides / daysPerSlide))});
						return r;
					};
				}(cont, daysPerSlide));
				
				//recalculate for mobile/resize
				if(spotter.mobile){
					spotter.mobile.onFontResize(cont.setSliderOffsets);
					spotter.mobile.onFontResize(cont.shift);
				}
				
				//spotter.events.swipeEvents(cont, '15%', 200, 'horizontal');
				spotter.events.expandedMouseEvents(cont);
				cont.addEventListener('swiperight', leftArrowClickEvent, false, {minSwipeDist:50});
				cont.addEventListener('swipeleft', rightArrowClickEvent, false, {minSwipeDist:50});
				cont.value = null;
				spotter.events.setEventTrigger(cont, 'change');
				spotter.events.setDelegatedEvent('click', cont, (function(cont){
					return function(){
						if(~this.className.indexOf('disabled')) return;
						var d = this.getAttribute('data-date');
						if(cont.value !== d){
							cont.unmarkSelected();
							cont.selectedDate = this;
							cont.value = d;
							cont.eventTriggers['change']();
						}
					}
				...
				cont.markSelected = function(){
					if(this.selectedDate) spotter.toggle.class.add(this.selectedDate, 'selected');
				}
				cont.unmarkSelected = function(){
					if(this.selectedDate) spotter.toggle.class.remove(this.selectedDate, 'selected');
				};
				cont.disableSelected = function(){
					if(this.selectedDate) spotter.toggle.class.add(this.selectedDate, 'disabled');
				};
			};
			__components.scheduler.templates = {
				column: '<div class="column">$</div>',
				label: '<div class="label">$</div>',
				activeBlock: '<div class="block $3" data-date="$1">$2</div>',
				disabledDay: '<div class="disabled day">$</div>'
			};
		// -- END SCHEDULER --
		...
		// -- TEMPLATE --	
			__components.spotterTemplate = (function(){				
				...
				registerSpotterComponent('spotter-template', {
					prototype: Object.create(HTMLScriptElement.prototype, {createdCallback: {value: __constructor}})
				});
			...
			__components.cropper=(function(){
				...
				var __constructor = function(cont){	
					...
					*/
					...
					shade.addEventListener('afterClose', deactivate);
					...
					//events
					frame.canvas.addEventListener(spotter.events.eventTypes.mouse.mousedown, function(e){
					...
					frame.canvas.addEventListener(spotter.events.eventTypes.mouse.mouseout, function(e){
						frame.active=false;
						handle.active=false;
					}, false);
					
					frame.canvas.addEventListener(spotter.events.eventTypes.mouse.mouseup, function(e){
						frame.active=false;
						handle.active=false;
					}, false);
					
					//frame move
					frame.canvas.addEventListener(spotter.events.eventTypes.mouse.mousemove, function(e){
		...
		// -- EXPANDABLE --
		__components.expandable = (function(){
			var ExpandableProto = Object.create(HTMLElement.prototype);
			
			//when created
			ExpandableProto.createdCallback = function(){
				if(isPartOfTemplate(this)) this.noProcess = true;
				if(!registerElem(this)) this.noProcess = true;
			};
			
			//when appended to DOM
			ExpandableProto.attachedCallback = function(){
				if(this.noProcess) return false;
				
				var temp;
			
				//expand button
				if(!(
					((temp = this.getAttribute('expand-button')) && (this.expandButton = document.getElementById(temp)))
					|| (this.expandButton = this.querySelector('.button.expand'))
				)){
					console.warn('spotter expandable elements require an expand button');
					this.noProcess = true;
					return false;
				}
				...
				//collapse button
				if(!(
					((temp = this.getAttribute('collapse-button')) && (this.collapseButton = document.getElementById(temp)))
					|| (this.collapseButton = this.querySelector('.button.collapse'))
				)){
					console.warn('spotter collapseable elements require a collapse button');
					this.noProcess = true;
					return false;
				}
				
				//content
				if(!(this.content = this.querySelector('.content'))){
					console.warn('spotter collapseable elements require content');
					this.noProcess = true;
					return false;
				}
				
				this.needToGetHeight = true;
				spotter.onResize(this.setHeights);
				
				this.expandButton.parentExpandable = this.collapseButton.parentExpandable = this;
			
				this.expandButton.addEventListener('click', this.expandContent);
				this.collapseButton.addEventListener('click', this.collapseContent);
				
				if(~this.expandButton.className.indexOf('hide')){
					this.expandContent();
				}
				else{
			...
			ExpandableProto.expandContent = function(){
				var toggle = spotter.toggle, cont = (this.tagName === "SPOTTER-EXPANDABLE" ? this : this.parentExpandable);
				toggle.hide(cont.expandButton);
				toggle.show(cont.collapseButton);
				
				if(cont.needToGetHeight){cont.setHeights();this.needToGetHeight = false;}
				Velocity(cont.content, {height: cont.contentHeight+'px', duration:500});
			};
			ExpandableProto.collapseContent = function(){
				var toggle = spotter.toggle, cont = (this.tagName === "SPOTTER-EXPANDABLE" ? this : this.parentExpandable);
				toggle.show(cont.expandButton);
				toggle.hide(cont.collapseButton);
				
				Velocity(cont.content, {height: 0, duration:500});
			};
			ExpandableProto.setHeights = function(){
				this.content.style.height = null;
				spotter.toggle.class.add(this.content, 'force-open');
				this.contentHeight = spotter.getHeight(this.content);
				spotter.toggle.class.remove(this.content, 'force-open');
			};
			
			//load the initial components and register them
			registerSpotterComponent('spotter-expandable', {prototype: ExpandableProto});
		}());
		// END EXPANDABLE
		...
		// SPOTTER-INPUTS//{
			...
			// TYPES
				...
				//STATIC-SELECT//{
					...
					/* structure html
					<spotter-input-menu name="input name" data-title="menu title">
						<div class="content">
							<div class="title">menu title</div>
							<ul class="list">
								<li class="list-item" value="A">Item A</li>
								<li class="list-item" value="B">Item B</li>
								...
							</ul>
						</div>
					</spotter-input-menu>
					*/
					...
					//events:
						//onchange - on new item click (item must be different from previous)
						//afterOpen - when something change the value to a non-empty value
						//afterClose - when something changes the value to an empty value
					
					//default class:						
						var StaticSelectProto = Object.create(HTMLElement.prototype);
						StaticSelectProto.createdCallback = function(){
							if(isPartOfTemplate(this)) return;
							if(!registerElem(this)) return;
							
							//setup structure
							var frag = document.createDocumentFragment()
								,inp = this.hiddenInput = document.createElement("INPUT")
								,list = this.list = document.createElement("UL")
								,cont = this.content = document.createElement("DIV")//for centering
								,opts = (this.options ? this.options.slice(0) : null);
							
							//default
							this.selected = null;
							this.value = this.getAttribute('default-value') || null;
							
							//hidden input to store menu value
							inp.type = "hidden";
							inp.name = this.getAttribute("name");
							inp.value = this.value;
							frag.appendChild(inp);
							
							//list container (ul element)
							list.className = "list";
							cont.appendChild(list);
							cont.className = 'animated';
							frag.appendChild(cont);
							
							//add default classes to the component, hide it initially, and append the content
							spotter.toggle.class.add(this, ["no-text-select"]);
							this.isOpen = false;
							this.appendChild(frag);
							
							//event hooks
							spotter.events.expandedMouseEvents(cont);
							//spotter.events.setEventTrigger(this, "open");//triggered programmatically by 'open' method
							//spotter.events.setEventTrigger(this, "close");//trigger programmatically by 'close' method or if all options are swiped away
							spotter.events.setEventTrigger(this, "change");//triggered by changing value on component
							spotter.events.setEventTrigger(this, "empty");//triggered when all options have been swiped away
							
							//triggered by swipeleft
							var removeChild = (function(_p){
								return function(){	
									this.removeEventListener('animationend', removeChild);
									var par = this.parentNode;
									spotter.deleteElement(this);
									if(par.children.length === 0){
										_p.eventTriggers['empty']();
										//_p.close();
									}
								};
							}(this));

							//delete element if user swipes it left
							spotter.events.setDelegatedEvent('swipeleft', cont, function(e){
								this.addEventListener('animationend', removeChild);
								this.className += ' slideOutLeft';
							}, ".list-item");
		
							//submit items as choice if user swipes right
							spotter.events.setDelegatedEvent('click', cont, (function(c){
								return function(e){
									if(c.selected){
										spotter.toggle.class.remove(c.selected, 'selected');
										if(c.selected === this){
											c.value = c.defaultValue||null;
											return;
										}
									}
									c.selected = this;
									spotter.toggle.class.add(c.selected, 'selected');
									c.value = this.getAttribute('value');
									//console.log('new value for input menu:', c.value, new Error().stack);
								};
							}(this)), ".list-item");
							
							//use component.options = [{name:string,value:string},...] to create the options within the menu - use component.options = (empty value) to remove all the options
							Object.defineProperty(this, "options", (function(){
								var options = [];
								return {
									set: function(newOpts){
										this.hiddenInput.value = null;
										if(!Array.isArray(newOpts)) newOpts = [newOpts];
										
										var list = this.list, l = newOpts.length, html, li, ch, c;
										if(!newOpts.length){
											list.removeChildren();
											this.close();
										}
											...
											html = [];
											for(var x = 0; x<l && (li=newOpts[x]); x++){
												/*
												ch = document.createElement('LI');
												ch.className = "list-item animated";
												ch.value = li['value'];
												t = document.createElement('DIV');
												t.className = "text";
												t.innerHTML = li['display'];
												ch.data = li;
												ch.appendChild(t);
												frag.appendChild(ch);
												*/
												html.push('<li class="list-item animated" value="'+li['value']+'"><div class="text">'+li['display']+'</div></li>');
											}
											list.innerHTML = html.join('');
											c = list.children;
											l = c.length;
											for(x=0;x<l && (li=newOpts[x]) && (ch=c[x]);x++){
												ch.data = li.data||null;
											}
										}
									},
									get: function(){
							...
							//if options were set previously, then will be activated now
							if(opts !== null) this.options = opts;
							
							//set value programmatically through component.value = [somevalue]
							Object.defineProperty(this, "value", {
								get: function(){
									return this.hiddenInput.value;
								},
								set: function(newValue){
									var oldValue = this.hiddenInput.value;
									//console.log('value change \n', new Error().stack);
									if(oldValue !== newValue){
										this.hiddenInput.value = newValue;
										this.eventTriggers['change']();
									}
								}
							});
							
							/*
							//since content is animated dont finalize the closing/opening of the component (which is mostly the shade) until animation complete - 'this' is the 'content'
							cont.addEventListener('animationend', (function(c){
								return function(){
									var list = this.list;
									console.log(c.isOpen);
									if(c.isOpen === "close"){	
										spotter.toggle.hide(c);
										this.eventTriggers['close']();
										c.isOpen = false;
									}
									else if(c.isOpen === "open"){
										c.isOpen = true;
									}
								};
							}(this)));
						...
						StaticSelectProto.attachedCallback = __components.isReady;
						/*
						//open shade, then animate open content - 'this' is the component
						StaticSelectProto.open = function(opts){
							if(this.isOpen !== false){ console.warn("select menu already open. if setting options use component.options = obj instead"); return; }
							this.isOpen = "open";
							if(opts){
								if(!Array.isArray(opts)){
									console.warn('spotter input type of select from list must be sent valid options to populate list where opts = [{name:string,value:string},...]');
									return false;
								}
								this.options = opts;
							}						
							this.eventTriggers['open']();
							spotter.toggle.show(this);
							//spotter.toggle.class.swap(this.content, 'zoomOut', 'zoomIn');
						};
						//animate close content and then close shade - 'this' is the component
						StaticSelectProto.close = function(){
							if(this.isOpen !== true){ console.warn("select menu is not currently open."); return; }
							this.isOpen = "close";
							//spotter.toggle.class.swap(this.content, 'zoomIn', 'zoomOut');
						};
						*/

					registerSpotterComponent('spotter-input-menu', {prototype:StaticSelectProto});
				//}
		//}
